  ...
  */
    public Hashtable upFile(HttpContextBase cxt, string pathbase, string[] filetype, int size)
    {
        ...
        try
        {
            ...
            {
                filename = reName();
                var imageSavePath=uploadpath + filename;
                uploadFile.SaveAs(imageSavePath);
                var index = imageSavePath.LastIndexOf(getFileExt());

                //生成89x68缩略图
                ImageHelper.MakeThumbnail(imageSavePath, imageSavePath.Insert(index,string.Format("_{0}x{1}",Global.LeanGardenThumbnailWidth,Global.LearGardenThumbnailHeight)), Global.LeanGardenThumbnailWidth, Global
                    .LearGardenThumbnailHeight, "Cut");
