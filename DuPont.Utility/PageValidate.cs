...
// ***********************************************************************
// <copyright file="PageValidate.cs" company="">
...
namespace DuPont.Utility
{
    public class PageValidate
    {
        ...
        private static Regex RegMobile = new Regex(@"^0*1\d{10}$");
        private static Regex RegIsSafePassword = new Regex(@"(\d+|[a-z]+|[A-Z]+|[-.!@#$%^&*()+?><]+){7,18}");
        private static Regex RegRemoveHtmlTag = new Regex(@"(&[^;]+;)|(nbsp;)|(\s)|(<[^>]+>)");
        ...
        /// <returns><c>true</c> if the specified phone number is mobile; otherwise, <c>false</c>.</returns>
        public static bool IsMobile(string phoneNumber)
        {
            return phoneNumber != null && RegMobile.IsMatch(phoneNumber);
        }
        ...
        public static bool IsSafePassword(string password)
        {
            ...
            //        && RegIsSafePassword.IsMatch(password);
            return Regex.IsMatch(password, "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[-.!@#$%^&*()+?><])[0-9a-zA-Z-.!@#$%^&*()+?><]{7,18}$");
        }

        #region "去除HTML标签和特殊字符"
        public static string RemoveHtmlTagAndSepecialChar(string htmlContent)
        {
            return RegRemoveHtmlTag.Replace(htmlContent, "");
        } 
        #endregion
    }
}
